...
const BASE_URL =
    'https://crewell.net/en/companies/?action=search&country%5B%5D=1';
...
const extractEmailFromImage = async imgSrc => {
    try {
        const {
            data: { text },
        } = await tesseract.recognize(imgSrc);
        return text.trim();
    } catch (error) {
...
const scrapeEmailsWithPagination = async () => {
    ...
    try {
        ...
        while (true) {
            ...
            for (const company of companyLinks) {
                ...
                console.log(
                    `Visiting company page: ${company.name} (${company.url})`,
                );
                try {
                    ...
                    const emailIndex = tableRows.findIndex(row =>
                        row.includes('E-mail:'),
                    );
                    let email = 'Not found';
            ...
            const nextPage = await page.$$eval(
                SELECTOR_PAGINATION,
                (paginationLinks, visitedArray) => {
                    const visitedSet = new Set(visitedArray);
                    for (const link of paginationLinks) {
                        const url = link.href;
                        if (!visitedSet.has(url)) return link.href;
                    }
                    return null;
                },
    ...
    const fileContent = allEmails
        .map(entry => `${entry.company}: ${entry.email}`)
        .join('\n');
