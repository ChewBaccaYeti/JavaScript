...
const BASE_URL =
    'https://crewdata.com/crewings.php?lang=eng&country=Ukraine&mode=companies';
...
const scrapeEmails = async () => {
    ...
    try {
        ...
        const paginationLinks = await page.$$eval(SELECTOR_PAGINATION, links =>
        ...
        for (const link of paginationLinks) {
            ...
            const pageEmails = await page.$$eval(SELECTOR_EMAIL, links =>
    ...
    const emailTable = emailArray.map((email, index) => ({
        Index: index + 1,
        Email: email,
    }));
