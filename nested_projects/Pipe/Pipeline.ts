...
import {
    Character,
    ApiResponse_Character,
    Starship,
    ApiResponse_Starships,
    Specie,
    ApiResponse_Species,
    ExtendedRequest,
} from './SWapiTypes';
...
async function fetchAllData<T>(url: string, limit: number): Promise<T[]> {
    ...
    while (nextUrl && results.length < limit) {
        const {
            next,
            results: newResults,
        }: { next: string | null; results: T[] } = await fetchWithCache<{
            next: string | null;
            results: T[];
        }>(nextUrl);
...
function fetchCharactersData(
    request: ExtendedRequest<ApiResponse_Character>,
    response: Response,
    next: NextFunction,
) {
...
function fetchStarshipsData(
    request: ExtendedRequest<ApiResponse_Starships>,
    response: Response,
    next: NextFunction,
) {
...
function fetchSpeciesData(
    request: ExtendedRequest<ApiResponse_Species>,
    response: Response,
    next: NextFunction,
) {
...
app.get(
    '/people',
    fetchCharactersData,
    (request: ExtendedRequest<ApiResponse_Character>, response: Response) => {
...
app.get(
    '/starships',
    fetchStarshipsData,
    (request: ExtendedRequest<ApiResponse_Starships>, response: Response) => {
        response.json(request.swapiData);
    },
);
...
app.get(
    '/species',
    fetchSpeciesData,
    (request: ExtendedRequest<ApiResponse_Species>, response: Response) => {
        response.json(request.swapiData);
    },
);
...
app.get('/search', (request: ExtendedRequest<any>, response: Response) => {
    ...
    const results = {
        characters:
            cache
                .get(people)
                ?.data.results.filter((char: Character) =>
                    char.name.toLowerCase().includes(query),
                ) || [],
        starships:
            cache
                .get(starships)
                ?.data.results.filter((ship: Starship) =>
                    ship.name.toLowerCase().includes(query),
                ) || [],
        species:
            cache
                .get(species)
                ?.data.results.filter((spec: Specie) =>
